...
export default function StageModal({
  ...
  const handleAnswer = (index: number) => {
    if (quizResult) return; // prevent double-click
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          <motion.div
            className="relative w-full max-w-lg rounded-2xl border p-6 z-10 max-h-[85vh] overflow-y-auto"
            style={{
          ...
          >
            ...
            <button
              onClick={handleClose}
              className="absolute top-3 right-3 text-mute-blue hover:text-light-gray transition text-xl z-10"
            >
              ✕
            </button>
            ...
            <div className="text-center mb-4">
              ...
              <p className="text-mute-blue text-xs mt-1">
            ...
            <AnimatePresence mode="wait">
              ...
              {phase === 'briefing' && (
                ...
                >
                  {/* NPC message */}
                  {stage.npcs[0] && (
                    <div className="bg-deep-navy/80 rounded-lg p-4 mb-4 border border-mute-blue/10">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-lg">🧑‍🔬</span>
                        <div>
                          <p className="text-xs font-bold" style={{ color: stage.color }}>{stage.npcs[0].name}</p>
                          <p className="text-[10px] text-mute-blue">{stage.npcs[0].role}</p>
                        </div>
                      </div>
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="rounded-lg p-3 border border-mute-blue/10 bg-deep-navy/50">
                      <p className="text-[10px] text-mute-blue font-bold tracking-wider mb-1">BRIEFING</p>
                      <p className="text-light-gray leading-relaxed">{stage.missionBriefing}</p>
                    </div>

                    <div className="rounded-lg p-3 border border-mute-blue/10 bg-deep-navy/50">
                      <p className="text-[10px] text-mute-blue font-bold tracking-wider mb-1">OBJECTIVE</p>
                      <p className="text-light-gray">{stage.missionObjective}</p>
                    </div>

                    <div className="rounded-lg p-3 border border-mute-blue/10 bg-deep-navy/50">
                      <p className="text-[10px] text-mute-blue font-bold tracking-wider mb-2">ACTIVITIES</p>
                      <ul className="space-y-1">
                        {stage.activities.map((act, i) => (
                          <li key={i} className="text-light-gray flex gap-2 text-xs">
                    ...
                    <div
                      className="rounded-lg p-3 text-center italic text-sm"
                  ...
                  {!isUnlocked && (
                    <button
                      ...
                      className="w-full mt-5 py-3 rounded-xl text-white font-title text-base hover:brightness-110 active:scale-95 transition-all"
              ...
              {phase === 'quiz' && (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    <span className="text-xs text-mute-blue font-ui">
                    ...
                    {quiz.choices.map((choice, i) => {
                      ...
                      return (
                        <motion.button
                          ...
                          className={`w-full text-left p-3 rounded-lg border ${borderColor} ${bg} text-sm text-light-gray font-ui hover:border-mute-blue/50 transition-all`}
                  ...
                  {quizResult === 'correct' && (
                    <motion.p
                      className="text-center text-green-400 text-sm mt-3 font-ui"
                  ...
                  {quizResult === 'wrong' && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, scale: 1 }}
                    >
                      <p className="text-red-400 text-sm font-ui">틀렸어요! 다시 도전!</p>
                      <p className="text-mute-blue text-xs mt-1">HINT: {quiz.hint}</p>
                    </motion.div>
                  )}
                  ...
                  <button
                    ...
                    className="mt-4 text-xs text-mute-blue hover:text-light-gray transition"
                  >
                    ← 브리핑으로 돌아가기
                  </button>
                </motion.div>
              ...
              {phase === 'authkey' && quizzesPassed && (
                ...
                >
                  <div className="bg-green-400/10 border border-green-400/30 rounded-lg p-4 mb-5">
                    <p className="text-green-400 font-title text-lg mb-1">
                      미션 퀴즈 통과!
                    </p>
                    <p className="text-sm text-light-gray font-ui">
